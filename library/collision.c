...
bounding_box_t polygon_bounding_box(const list_t *poly) {
    assert(list_size(poly) > 0);
    vector_t point0 = list_copy_vector(poly, 0);
    bounding_box_t box = {.lower_left = point0, .upper_right = point0};

    for (size_t i = 1; i < list_size(poly); i++) {
        vector_t point = list_copy_vector(poly, i);
...
interval_t polygon_project(const list_t *poly, vector_t axis) {
    vector_t point0 = list_copy_vector(poly, 0);
...
partial_sat_result_t partial_sat(const list_t *poly1, const list_t *poly2) {
    vector_t min_axis = VEC_ZERO;
    double min_intersect = DBL_MAX;
    // employ separating axis theorem
    for (size_t i = 0; i < list_size(poly1); i++) {
        vector_t v1 = list_copy_vector(poly1, i);
        vector_t v2 = list_copy_vector(poly1, (i + 1) % list_size(poly1));
...
collision_info_t find_collision(const list_t *poly1, const list_t *poly2) {
