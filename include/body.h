...
/**
 * Adds a decal to the body.
 * Contract: the decal body should be massless and not have any forces tied to it.
 * The decal needs to be added to the scene separately from the body
 * and after the body is added.
 *
...
/**
 * Translates all vertices in a body's polygon shape by the given vector.
 * Also changes the body's centroid by the given vector.
 * Note: mutates the original polygon.
 *
 * @param body a pointer to a body returned from body_init()
 * @param translation the vector to add to each vertex's position and the centroid
 */
 ...
 *
 * @param body a pointer to a body returned from body_init()
 * @param v the body's new acceleartion
 */
void body_set_acceleration(body_t *body, vector_t v);

/**
 * Changes a body's angular velocity (the time-derivative of its theta).
 *
 * @param body a pointer to a body returned from body_init()
