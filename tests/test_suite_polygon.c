...
void test_square_translate() {
    ...
    assert(vec_equal(*list_get_vector(sq, 0), (vector_t) {3, 4}));
    assert(vec_equal(*list_get_vector(sq, 1), (vector_t) {1, 4}));
    assert(vec_equal(*list_get_vector(sq, 2), (vector_t) {1, 2}));
    assert(vec_equal(*list_get_vector(sq, 3), (vector_t) {3, 2}));
    assert(isclose(polygon_area(sq), 4));
...
void test_square_rotate() {
    ...
    assert(vec_isclose(*list_get_vector(sq, 0), (vector_t) {0, sqrt(2)}));
    assert(vec_isclose(*list_get_vector(sq, 1), (vector_t) {-sqrt(2), 0}));
    assert(vec_isclose(*list_get_vector(sq, 2), (vector_t) {0, -sqrt(2)}));
    assert(vec_isclose(*list_get_vector(sq, 3), (vector_t) {sqrt(2), 0}));
    assert(isclose(polygon_area(sq), 4));
    assert(vec_isclose(polygon_centroid(sq), VEC_ZERO));
    list_free(sq);
}
...
void test_triangle_translate() {
    ...
    assert(vec_equal(*list_get_vector(tri, 0), (vector_t) {-4, -3}));
    assert(vec_equal(*list_get_vector(tri, 1), (vector_t) {0,  -3}));
    assert(vec_equal(*list_get_vector(tri, 2), (vector_t) {0,  0}));
    assert(isclose(polygon_area(tri), 6));
...
void test_triangle_rotate() {
    ...
    assert(vec_isclose(*list_get_vector(tri, 0), (vector_t) {-1,  3}));
    assert(vec_isclose(*list_get_vector(tri, 1), (vector_t) {2.2, 0.6}));
    assert(vec_isclose(*list_get_vector(tri, 2), (vector_t) {4,   3}));
    assert(isclose(polygon_area(tri), 6));
...
void test_weird_translate() {
    ...
    assert(vec_isclose(*list_get_vector(w, 0), (vector_t) {-10, -20}));
    assert(vec_isclose(*list_get_vector(w, 1), (vector_t) {-6,  -19}));
    assert(vec_isclose(*list_get_vector(w, 2), (vector_t) {-12, -19}));
    assert(vec_isclose(*list_get_vector(w, 3), (vector_t) {-15, -15}));
    assert(vec_isclose(*list_get_vector(w, 4), (vector_t) {-11, -28}));
    assert(isclose(polygon_area(w), 23));
...
void test_weird_rotate() {
    ...
    assert(vec_isclose(*list_get_vector(w, 0), (vector_t) {2,  2}));
    assert(vec_isclose(*list_get_vector(w, 1), (vector_t) {1,  6}));
    assert(vec_isclose(*list_get_vector(w, 2), (vector_t) {1,  0}));
    assert(vec_isclose(*list_get_vector(w, 3), (vector_t) {-3, -3}));
    assert(vec_isclose(*list_get_vector(w, 4), (vector_t) {10, 1}));
    assert(isclose(polygon_area(w), 23));
...
// void test_collision() {
//     list_t *w1 = make_weird();
//     list_t *w2 = make_weird();
//     assert(polygon_colliding(w1, w2));
//     polygon_translate(w2, (vector_t){1, 0});
//     assert(polygon_colliding(w1, w2));
//     polygon_translate(w2, (vector_t){0, 1});
//     assert(polygon_colliding(w1, w2));
//     polygon_translate(w2, (vector_t){0, 7});
//     assert(polygon_colliding(w1, w2));
//     polygon_translate(w2, (vector_t){4, 1}); // now only touching at one point
//     assert(polygon_colliding(w1, w2));
//     polygon_translate(w2, (vector_t){1, 1});
//     assert(!polygon_colliding(w1, w2));
//     polygon_translate(w2, (vector_t){10, 10});
//     assert(!polygon_colliding(w1, w2));
//     list_free(w1);
//     list_free(w2);
// }

// void test_right_cleared() {
//     list_t *w1 = make_weird();
//     list_t *w2 = make_weird();
//     assert(!polygon_right_cleared(w1, w2));
//     polygon_translate(w2, (vector_t){1, 0});
//     assert(!polygon_right_cleared(w2, w1));
//     polygon_translate(w2, (vector_t){7, 0});
//     assert(!polygon_right_cleared(w2, w1));
//     polygon_translate(w2, (vector_t){1, 0});
//     // TODO(BRUNO) NOT WORKING
//     // assert(!polygon_right_cleared(w2, w1));
//     // polygon_translate(w2, (vector_t){0, 10});
//     // assert(!polygon_right_cleared(w2, w1));
//     // polygon_translate(w2, (vector_t){0, -25});
//     // assert(!polygon_right_cleared(w2, w1));
//     // polygon_translate(w2, (vector_t){0.00001, 0});
//     // assert(polygon_right_cleared(w2, w1));
//     // polygon_translate(w2, (vector_t){10, 0});
//     // assert(polygon_right_cleared(w1, w2));
//     list_free(w1);
//     list_free(w2);
// }
