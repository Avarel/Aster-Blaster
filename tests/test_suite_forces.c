...
void test_spring_sinusoid() {
    ...
    for (int i = 0; i < STEPS; i++) {
        // printf("Expected x: %.7f, Actual x: %.7f, Steps: %d\n",A * cos(sqrt(K / M) * i * DT), body_get_centroid(mass).x, i);
        // assert(vec_isclose(
        //     body_get_centroid(mass),
        //     (vector_t) {A * cos(sqrt(K / M) * i * DT), 0}
        // ));
        assert(vec_within(
            1e-6, body_get_centroid(mass),
            (vector_t) {A * cos(sqrt(K / M) * i * DT), 0}
        ));
...
void test_energy_conservation() {
    ...
    for (int i = 0; i < STEPS; i++) {
        ...
        // printf("Intial Energy: %.5f, Current Energy: %.5f, Ratio: %.5f, Steps: %d\n", initial_energy, energy, energy / initial_energy, i);
...
void test_collisions() {
    ...
    for (int i = 0; i < TICKS_TO_COLLISION * 2; i++) {
        ...
        if (i < TICKS_TO_COLLISION) {
            ...
            scene_tick(scene, DT);
            // printf("Num of bodies: %zu", scene_bodies(scene));
