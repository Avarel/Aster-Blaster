...
void test_body_init() {
    ...
    list_t *shape2 = body_get_shape_cloned(body);
...
void test_body_setters() {
    ...
    *v = (vector_t) {-1, 0};
    ...
    shape = body_get_shape_cloned(body);
...
void test_forces() {
    ...
    *v = (vector_t) {-1, 0};
    ...
    vector_t new_centroid =
        vec_multiply(DT, new_velocity);
    assert(vec_isclose(body_get_centroid(body), new_centroid));
    body_tick(body, DT);
    vector_t newer_velocity =
        vec_add(new_velocity, (vector_t) {6 * DT, 8 * DT});
    assert(vec_isclose(
        body_get_centroid(body),
        vec_add(new_centroid, vec_multiply(DT, newer_velocity))
    ));
    body_free(body);
}
...
void test_body_info() {
    ...
    *info = 123;
    body_t *body = body_init_with_info(shape, 1, (rgb_color_t) {0, 0, 0}, info, free); //changed freer from NULL to free
    assert(*(int *) body_get_info(body) == 123);
    body_free(body);
    // free(info); //redundant freeing
}
