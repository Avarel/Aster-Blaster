...
void test_list_size1() {
    ...
    *v = VEC_ZERO;
    ...
    list_add(l, v);
    assert(list_size(l) == 1);
    assert(vec_equal(*list_get_vector(l, 0), (vector_t) {1, 1}));
    // Modify
    *v = (vector_t) {1, 2};
    assert(list_size(l) == 1);
    assert(vec_equal(*list_get_vector(l, 0), (vector_t) {1, 2}));
    list_free(l);
}
...
void test_list_small() {
    ...
    *v = VEC_ZERO;
    ...
    assert(vec_equal(*list_get_vector(l, 0), VEC_ZERO));
    assert(vec_equal(*list_get_vector(l, 1), (vector_t) {1, 1}));
    assert(vec_equal(*list_get_vector(l, 2), (vector_t) {2, 2}));
    // Fill to capacity
    v = malloc(sizeof(*v));
    v->x = v->y = 3;
    list_add(l, v);
    ...
    assert(vec_equal(*list_get_vector(l, 3), (vector_t) {3, 3}));
    assert(vec_equal(*list_get_vector(l, 4), (vector_t) {4, 4}));
    // Remove some
    v = list_remove(l, 4);
    ...
    list_add(l, v);
    assert(list_size(l) == 5);
    assert(vec_equal(*list_get_vector(l, 0), (vector_t) {0, 0}));
    assert(vec_equal(*list_get_vector(l, 1), (vector_t) {1, 1}));
    assert(vec_equal(*list_get_vector(l, 2), (vector_t) {2, 2}));
    assert(vec_equal(*list_get_vector(l, 3), (vector_t) {5, 5}));
    assert(vec_equal(*list_get_vector(l, 4), (vector_t) {6, 6}));
    // Overwrite added elements
    *list_get_vector(l, 3) = (vector_t) {7, 7};
    *list_get_vector(l, 4) = (vector_t) {8, 8};
    assert(vec_equal(*list_get_vector(l, 0), (vector_t) {0, 0}));
    assert(vec_equal(*list_get_vector(l, 1), (vector_t) {1, 1}));
    assert(vec_equal(*list_get_vector(l, 2), (vector_t) {2, 2}));
    assert(vec_equal(*list_get_vector(l, 3), (vector_t) {7, 7}));
    assert(vec_equal(*list_get_vector(l, 4), (vector_t) {8, 8}));
    list_free(l);
}
...
void test_list_large_get_set() {
    ...
    // Check
    for (size_t i = 0; i < LARGE_SIZE; i++) {
        assert(vec_equal(*list_get_vector(l, i), (vector_t) {i, i}));
    }
    // Set every 100th value
    for (size_t i = 0; i < LARGE_SIZE; i += 100) {
        vector_t *v = list_get_vector(l, i);
        v->x = v->y = i * 10;
    }
...
void test_list_large_add_remove() {
    ...
    for (size_t i = 1; i <= LARGE_SIZE; i++) {
        ...
        vector_t *v = list_remove(l, value);
    ...
    // Check all
    for (size_t i = 0; i < LARGE_SIZE; i++) {
        assert(vec_equal(*list_get_vector(l, i), (vector_t) {i + 1, i + 1}));
    }
    list_free(l);
}
...
void get_out_of_bounds(void *aux) {
    ...
    list_get_vector(access->list, access->index);
}
void test_out_of_bounds_access() {
    ...
    for (size_t size = 0; size <= max_size; size++) {
        ...
        for (access->index = 0; access->index < size; access->index++) {
            ...
            *list_get_vector(access->list, access->index) = new_vector;
            assert(vec_equal(*list_get_vector(access->list, access->index), new_vector));
        }
...
void test_full_add() {
    ...
    // There should not be any resizes yet
    assert(list_capacity(l) == size);

    // Try adding to the full list -- should NOT fail
    add_past_end(l);
    // Capacity should have doubled given that the impl growth factor is 2
...
void test_remove_index() {
    ...
    // Fill list with copies of v, then remove them all
    for (size_t i = 0; i < size; i++) {
        list_add(l, vec_alloc(vec(i, i)));
    }
    for (size_t i = 0; i < size; i++) {
        vector_t *list_v = list_remove(l, 0);
        assert(vec_equal(*list_v, vec(i, i)));
        free(list_v);
    }
